...
function preliminaryUpdates($getPage_connection2) {
	...
	if ($limit_allClaims >= 1) {
		$next_claims = 1;
		if ($stmt = $getPage_connection2->prepare("SELECT id FROM claims ORDER BY id ASC LIMIT 1")) {
			$stmt->execute();
			$stmt->bind_result($r_result);
			$stmt->fetch();
		...
		while ($next_claims > 0) {
			...
			if ($stmt = $getPage_connection2->prepare("SELECT id FROM claims WHERE id = (SELECT MIN(id) FROM claims WHERE id > ?) ORDER BY id LIMIT 1")) {
				$stmt->bind_param("i", $next_claims);
				$stmt->execute();
				$stmt->bind_result($r_result);
				$stmt->fetch();
				$next_claims = $r_result;
...
function updateNations($getPage_connection2) {
	...
	while ($next_nations > 0) {
		...
		********************************/
		...
		if ($stmt = $getPage_connection2->prepare("SELECT id FROM nations WHERE id = (SELECT MIN(id) FROM nations WHERE id > ?) ORDER BY id LIMIT 1")) {
			$stmt->bind_param("i", $next_nations);
			$stmt->execute();
			$stmt->bind_result($r_result);
			$stmt->fetch();
...
function updateGlobe($getPage_connection2) {
	...
	$next_organizations = 1;
	if ($stmt = $getPage_connection2->prepare("SELECT id FROM organizations ORDER BY id ASC LIMIT 1")) {
		$stmt->execute();
		$stmt->bind_result($r_result);
		$stmt->fetch();
		...
		$next_organizations = 0;
	} // else
	while ($next_organizations > 0) {
		...
		if ($stmt = $getPage_connection2->prepare("SELECT id FROM organizations WHERE id = (SELECT MIN(id) FROM organizations WHERE id > ?) ORDER BY id LIMIT 1")) {
			$stmt->bind_param("i", $next_organizations);
			$stmt->execute();
			$stmt->bind_result($r_result);
			$stmt->fetch();
			$next_organizations = $r_result;
