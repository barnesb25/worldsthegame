	...
	if ($_SESSION["admin"] == 1) {
		if (strlen($_SESSION["action"]) >= 1) {
			if ($_SESSION["action"] == "reset_world") {										
				...
				if ($stmt = $getPage_connection3->prepare("SELECT id FROM continents ORDER BY id ASC LIMIT 1")) {
					$stmt->execute();
				...
				while ($next_continents > 0) {
					...
					$next_tiles = 1;
					$counter1 = 0;
					if ($stmt = $getPage_connection3->prepare("SELECT id FROM tilesmap ORDER BY id ASC LIMIT 1")) {
						$stmt->execute();
						$stmt->bind_result($r_result);
						$stmt->fetch();
						$next_tiles = $r_result;
					...
					while ($next_tiles > 0) {
						...
						if ($stmt = $getPage_connection3->prepare("SELECT id FROM tilesmap WHERE id = (SELECT MIN(id) FROM tilesmap WHERE id > ?) ORDER BY id LIMIT 1")) {
					...
					if ($stmt = $getPage_connection3->prepare("SELECT id FROM continents WHERE id = (SELECT MIN(id) FROM continents WHERE id > ?) ORDER BY id LIMIT 1")) {
